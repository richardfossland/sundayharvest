 ...
 * shared Sunday Account auth project — NOT the app's own DATA project (the
 * `harvest`-schema game DB). It is used only to resolve the signed-in host
 * (`auth.getUser()`); authorization then happens in `lib/server/auth-host.ts`
 * against HARVEST_ADMIN_EMAILS.
 */
