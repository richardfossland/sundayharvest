#!/usr/bin/env bash
# Validate the SundayHarvest migration + game logic against a throwaway Postgres.
...
echo "→ migration (1st apply)"; run supabase/migrations/0001_harvest_schema.sql
...
echo "$OUT" | grep -q "ALL GAME-LOGIC TESTS PASSED" || { echo "TESTS FAILED"; echo "$OUT" | tail -20; exit 1; }
